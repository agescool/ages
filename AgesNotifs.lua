...
if Core:FindFirstChild("AgesNotifications") then Core:FindFirstChild("AgesNotifications"):Destroy() end
local UI = New("ScreenGui", {
	["IgnoreGuiInset"] = true; ["Name"] = "AgesNotifications"; ["ResetOnSpawn"] = false; ["ZIndexBehavior"] = "Global"
...
function Ages:TextConstraint(Item)
	New("UITextSizeConstraint", {Parent = Item, MaxTextSize = Item["TextSize"]}); Item["TextScaled"] = true
end

function Ages:ApplyTheme(Theme, Frame)
    local Desc = Frame:GetDescendants()
    if Ages["Themes"][Theme:lower()] then
        for i,v in next, Ages["Themes"][Theme:lower()] do
...
function Ages:Notify(Title, Message, Type, Duration)
    Ages["Config"] = _G.Config
	...
	local Frame = New("Frame", {Name = "MainFrame", Parent = NotificationHolder, BackgroundColor3 = Color3.fromRGB(21, 21, 21),
	...
	local Header = New("TextLabel", {Parent = Frame, BackgroundTransparency = 1, Position = UDim2.new(0.237, 0, 0.172, 0), Size = UDim2.new(0.4, 0, 0.358, 0), ZIndex = 2, Font = "GothamMedium",
		Text = Title or "Anti by ages", TextSize = 14, TextXAlignment = "Left", TextScaled = true, TextColor3 = Color3.new(1, 1, 1)
	}); Ages:TextConstraint(Header);
    ...
    Ages:ApplyTheme((Ages["Config"]["Theme"]), Frame)
    if Ages["Config"]["Theme"]:lower() == "light" and Type:lower() == "normal" then 
